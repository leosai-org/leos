...
| Artifact version and dependency lifecycle | Immutable versioning, provenance, exact revision linkage, semantic-version constraints, and fail-closed dependency conformance are defined | Generic Artifact object, package envelope, publishing, dependency resolution/lock, acquisition, deprecation, and revocation owners | Contract or validator output must not become a package manager or publication authority. |
| Installed artifact instance | Plugin Installation and Plugin Activation target contracts are distinct | Install, configure, activate, disable, update, rollback, remove, transaction, event, and data-ownership authority | Installation and activation must not imply trust, permission, approval, eligibility, resolution, or execution. |
...
| Tool identity and operation catalog | Canonical Tool Definition shape now covers identity, schemas, capability linkage, risk, side effects, idempotency, timeout, cancellation, runtime, trust, revision, and audit; Dispatcher is invocation authority | Tool definition/catalog lifecycle and ingestion owner | Tool, Plugin, Tool Runtime, or adapter must not invoke outside Dispatcher. |
| Capability Profile lifecycle | Profile target contracts distinguish Organization, Team, Employee, and Runtime scope and remain non-authoritative configuration | Owners, inheritance, policy interpretation, editing, activation, and compatibility with Ranking Policy Authority | Profiles do not grant, approve, establish candidate order, select, resolve, or invoke; ranking remains `job > employee > capability > global`. |
| Runtime compatibility evidence | Evidence shape is scoped, revision-pinned, attributable, time-bounded, and explicitly non-authoritative | Observer authority, freshness, reconciliation, runtime-target ownership, and production ingestion | Evidence may inform eligibility but never installs, activates, authorizes, approves, ranks, resolves, or executes. |
| Plugin ecosystem revocation | Exact subject/revision and required effect shape are defined; Artifact Trust Authority verifies artifact evidence only | Subject-specific declarers, policy, verification, propagation, in-flight treatment, recovery, and persistence | Artifact Trust does not acquire general publishing, installation, activation, Tool, Provider, or execution authority. |
