...
def main()->int:
 ...
 with tempfile.TemporaryDirectory(prefix='leos-operator-accept-') as td:
  ...
  manifest=read_json(root/'examples/installation-manifest.json'); manifest['target_root']=target.as_posix(); manifest['files']={'state/installation-plan.json':{'mode':'0o640','sha256':op.sha256_file(plan_path),'size_bytes':plan_path.stat().st_size}}
  ...
  commands={'status':['status'],'doctor':['doctor'],'services':['services'],'logs':['logs','--service','ai-router'],'backup':['backup','--destination','backups/operator.tar.gz'],'update':['update','--target-release','0.1.0-dev-preview-rc10'],'version':['version']}
  results={}
